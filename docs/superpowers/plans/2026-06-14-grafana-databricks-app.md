...
- [ ] **Step 1: Implement `startup.py`** sequence:
  ...
  6. Write `pgbouncer.ini` + `userlist.txt` (render fns), then **`os.chmod(path, 0o600)`** on both (they contain the token + local password) — the render fns are pure and can't enforce this, so the supervisor must. Before rendering `userlist.txt`, assert the generated `client_pw` contains no `"` (it won't: `secrets.token_urlsafe` is quote-free — assert the contract explicitly). Define `write_server_cred(tok)` = rewrite the `[databases]` line's `password=` in `pgbouncer.ini`, preserving `0600`. Never pass the token on a subprocess command line (would leak via `ps`). Launch PgBouncer (`bin/pgbouncer`).
  7. Write `provisioning/datasources/lakebase.yaml` from `render_lakebase_datasource(...)`, then `os.chmod(path, 0o600)` (contains the local password).
...
`ep-example-00000000.database.us-east-1.cloud.databricks.com:5432`, DB `grafana`,
