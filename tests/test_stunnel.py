...
def test_render_conf_sets_sni_to_lakebase_host_in_client_mode():
    conf = render_conf(accept_port=5433,
                       server_host="ep-lucky-tree-d25raxtv.database.us-east-1.cloud.databricks.com",
                       server_port=5432)
    assert "client = yes" in conf
    assert "accept = 127.0.0.1:5433" in conf
    assert ("connect = ep-lucky-tree-d25raxtv.database.us-east-1.cloud.databricks.com:5432"
            in conf)
    # SNI must be the Lakebase hostname — this is the whole reason stunnel exists.
    assert ("sni = ep-lucky-tree-d25raxtv.database.us-east-1.cloud.databricks.com"
            in conf)
